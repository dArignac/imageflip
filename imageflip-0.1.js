var ImageFlip = new Class({
		...
		new Asset.images(imagesToLoad, {
			onProgress: function(idx) {
				...
				if(idx < imagesToLoad.length) {
					innerText.setText(loadingText.replace('#IMG#', idx + 1));
				}
				this.setStyles({
					width: '100%',
					height: '100%'
	...
	finalizeImageLoading: function() {
		...
		$('_preloadingDiv_').effect('opacity', {duration: 500, transition: Fx.Transitions.linear}).start(1, 0).chain(this.addImageElements.bind(this));
	},
	...
	addImageElements: function() {
		...
		$(this.elements[0]).effect('opacity', {duration: 700, transition: Fx.Transitions.Expo.easeIn}).start(0, 1).chain(this.fadeInElement.bind(this, [1]));
	},
	...
	fadeInElement: function(idx) {
		if (idx < this.elements.length) {
			$(this.elements[idx]).effect('opacity', {duration: 700, transition: Fx.Transitions.Expo.easeIn}).start(0, 1).chain(this.fadeInElement.bind(this, [++idx]));
		}
	},
	...
	moveNext: function(e) {
		...
		if (!this.block) {
			...
			var fade = $(this.elements[this.elements.length - 1]).effect('opacity', {duration: 1000, transition: Fx.Transitions.Circ.easeInOut});					
	...
	movePrevious: function(e) {
		...
		if (!this.block) {
			...
			// fade out last elements
			var fade = $(this.elements[0]).effect('opacity', {duration: 1000, transition: Fx.Transitions.Circ.easeInOut});					
